.PHONY: provision_sandbox provision_devbox provision_sandbox_last

provision_sandbox:
	ansible-playbook -i hosts/sandbox --vault-password-file ~/Dropbox/www/pt-ops/vault-key server-sandbox.yml

provision_sandbox_last:
	ansible-playbook -i hosts/sandbox --vault-password-file ~/Dropbox/www/pt-ops/vault-key --tags after-roles server-sandbox.yml
