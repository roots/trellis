...
module Trellis
  class Config
    ...
    def site_hosts
      @site_hosts ||= begin
        apps.flat_map { |(_name, site)| site['site_hosts'] }.tap do |hosts|
    ...
    def apps
      @apps ||= begin
        content['apps'].tap do |sites|
          fail_with message: "No apps found in #{path}." if sites.to_h.empty?
        end
      end
    end
    ...
    def path
      File.join(@root_path, 'group_vars', 'development', 'apps.yml')
    end
